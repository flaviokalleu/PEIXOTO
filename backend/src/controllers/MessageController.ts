...
export const addReaction = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { emoji, reactionType } = req.body; // Aceita ambos os campos
  
  // Use emoji se disponível, senão use reactionType
  const reaction = emoji || reactionType;
  
  if (!reaction) {
    throw new AppError("Reaction emoji is required", 400);
  }
